...
var Logsedit = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('logshistory/logsedit/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('logshistory/logsedit/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('logshistory/logsedit/update');
	...
	confirmDelete: function (pLogId,  irow){
		$('[name="encrypt_log_id"]').val(pLogId);
	...
	deleteRecord: function(){
		var frm_action = site_url('logshistory/logsedit/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Logsedit.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Logsedit.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Logsedit.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Logsedit.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Logsedit.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pLogId = $(this).attr('data-log_id');

		Logsedit.confirmDelete(pLogId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Logsedit.deleteRecord();
	});
	setDropdownList('#log_edit_user');
