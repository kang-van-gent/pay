...
var Company = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('backend/company/preview_ajax/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('backend/company/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('backend/company/update');
	...
	confirmDelete: function (pCompanyId,  irow){
		$('[name="encrypt_company_id"]').val(pCompanyId);
	...
	deleteRecord: function(){
		var frm_action = site_url('backend/company/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Company.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Company.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Company.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Company.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Company.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		var pCompanyId = $(this).attr('data-company_id');

		Company.confirmDelete(pCompanyId,  row_num);
	});//click
