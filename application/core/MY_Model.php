...
class MY_Model extends CI_Model
{
    ...
    {
		if($this->_where == ''){
			$this->error_message = 'Choose WHERE Clause for Update / Delete';
			return false;
		}else{
		...
		if($pk_field = $this->get_primary_key()){
			...
			foreach ($query->result_array() as $row){
				...
				$data = array(
					...
					,'log_edit_br' => $log_table_id
					,'log_login_ip' => $_SERVER['REMOTE_ADDR']
					
				);
				$this->db->set($data);
				$this->db->insert('tb_logsedit');
				$n++;
			}
			return $n;
		}
	...
	public function createOptionList($table, $field_value, $field_text, $condition = array())
	{
		...
		$sql = "SELECT $field_value, $field_text $field_attributes FROM $table $where $order_by";
