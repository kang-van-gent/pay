...
class Company extends CRUD_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('user/Company_model', 'Company');
		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('user/company');		
		...
		$js_url = 'assets/js_modules/user/company.js?ft='. filemtime('assets/js_modules/user/company.js');
	...
	public function search()
	{
		...
		$page_url = site_url('user/company');
		...
		$this->render_view('user/company/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Company', 'url' => site_url('user/company')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('user/company/preview_view');
			}
		}
	}
	...
	public function preview_ajax($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->setPreviewFormat($results);
				$this->parser->parse('user/company/preview_view', $this->data);
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('user/company/add_view'); 
	}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->setPreviewFormat($results);
				$this->render_view('user/company/edit_view');
			}
		}
	}
