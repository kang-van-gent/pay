...
class Secbranch extends CRUD_Controller
{
	...
	public function formValidate()
	{
		...
		$frm->set_rules('secbranch_code', 'รหัสแผนก', 'trim|required');
		$frm->set_rules('secbranch_name', 'แผนก', 'trim|required');
		$frm->set_rules('sec_branch_code', 'สาขา', 'trim|required');
		$frm->set_rules('sec_void', 'สถานะ[0=open,1=close]', 'trim|required|is_natural');
	...
	public function formValidatenameId()
	{
		...
		if($count > 0){
			$message .= "รหัส $secbranch_code ซ้ำกัน";
		}
		return $message;
	}
	...
	public function update()
	{
		...
		if ($message != '') {
			...
			if($result == false){
				...
				$message = '<strong>บันทึกข้อมูลเรียบร้อย</strong>' . $this->Secbranch->error_message;
				$ok = TRUE;
			}
			$json = json_encode(array(
						'is_successful' => $ok,
	...
	public function export_excel() 
	{
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'รหัสแผนก');
		$objPHPExcel->getActiveSheet()->SetCellValue('B1', 'แผนก');
