...
/**
 * Posbranch_model Class
 * @date 2020-06-18
 */
class Posbranch_model extends MY_Model
{
	...
	{
		$posbranch_id = checkEncryptData($data['posbranch_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.posbranch_id = $posbranch_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.posbranch_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'posbranch_code' => $post['posbranch_code']
				,'posbranch_name' => $post['posbranch_name']
				,'pos_branch_code' => $post['pos_branch_code']
				,'pos_void' => $post['pos_void']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'posbranch_code'){
				$search_method_value = "LIKE '%$value%'";				
			}
			if($search_field == 'posbranch_name'){
				$search_method_value = "LIKE '%$value%'";				
			}
			if($search_field == 'pos_branch_code'){
				$search_method_field = "tb_branch_1.branch_nick";
				$search_method_value = "LIKE '%$value%'";				
			}
		...
		if ($where != '') {
			$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.pos_branch_code = tb_branch_1.branch_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_branch_1.branch_nick AS posBranchCodeBranchNick
				");
		$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.pos_branch_code = tb_branch_1.branch_id", 'left');

		$list_record = $this->list_record($order_by);
		$data = array(
				'total_row'	=> $total_row, 
	...
	public function update($post)
	{
		...
		$posbranch_id = checkEncryptData($post['encrypt_posbranch_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.posbranch_id = $posbranch_id");
