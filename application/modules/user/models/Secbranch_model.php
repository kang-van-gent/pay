...
/**
 * Secbranch_model Class
 * @date 2020-06-18
 */
class Secbranch_model extends MY_Model
{
	...
	{
		$secbranch_id = checkEncryptData($data['secbranch_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.secbranch_id = $secbranch_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.secbranch_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'secbranch_code' => $post['secbranch_code']
				,'secbranch_name' => $post['secbranch_name']
				,'sec_branch_code' => $post['sec_branch_code']
				,'sec_void' => $post['sec_void']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'secbranch_code'){
				$search_method_value = "LIKE '%$value%'";				
			}
			if($search_field == 'secbranch_name'){
				$search_method_value = "LIKE '%$value%'";				
			}
			if($search_field == 'sec_branch_code'){
		...
		if ($where != '') {
			$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.sec_branch_code = tb_branch_1.branch_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_branch_1.branch_nick AS secBranchCodeBranchNick
				");
		$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.sec_branch_code = tb_branch_1.branch_id", 'left');
	...
	public function update($post)
	{
		...
		$secbranch_id = checkEncryptData($post['encrypt_secbranch_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.secbranch_id = $secbranch_id");
