...
/**
 * Depbranch_model Class
 * @date 2020-06-18
 */
class Depbranch_model extends MY_Model
{
	...
	{
		$depbranch_id = checkEncryptData($data['depbranch_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.depbranch_id = $depbranch_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.depbranch_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'depbranch_code' => $post['depbranch_code']
				,'depbranch_name' => $post['depbranch_name']
				,'dep_branch_code' => $post['dep_branch_code']
				,'dep_void' => $post['dep_void']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($search_field != '' && $value != ''){
			...
			$search_method_value = '';
			if($search_field == 'depbranch_code'){
				$search_method_value = "LIKE '%$value%'";				
			}
			if($search_field == 'depbranch_name'){
				$search_method_value = "LIKE '%$value%'";				
			}
			if($search_field == 'dep_branch_code'){
		...
		if ($where != '') {
			$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.dep_branch_code = tb_branch_1.branch_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_branch_1.branch_nick AS depBranchCodeBranchNick
				");
		$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.dep_branch_code = tb_branch_1.branch_id", 'left');
	...
	public function update($post)
	{
		...
		$depbranch_id = checkEncryptData($post['encrypt_depbranch_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.depbranch_id = $depbranch_id");
