...
<div class="card">
	<div class="card-header bg-info">
		<h3 class="card-title"><i class="fa fa-list-alt"></i><b> บันทึกรายได้ประจำงวด</b></h3>
	</div>
	<div class="card-body">
		<div class="row">
			...
			<div class="col-sm-12 col-md-9">
				...
				<div class="input-group input-group-button">
					...
					<option value="rf_month_id">เดือน</option>
						</select>
 					</div>
					<div class="form-group">
			...
			<table id="simpletable" class="table table-striped table-bordered nowrap">
				...
				<tbody>
					<tr parser-repeat="[data_list]" id="row_{record_number}">
						...
						<td>{ahead_id}</td>
						<td>{rfPersonName} {rfPersonSurname} </td>
						<td>{rfPrlistName}</td>
						<td>{rfPaymonthMonth}</td>
						<td>{rfPaynumDetails}</td>
						<td>{ahead_pay}</td>
...
<!-- 								<a href="javascript:void(0);" class="btn-delete-row my-tooltip btn btn-danger btn-sm"
									data-toggle="tooltip" title="ลบรายการนี้"
									 data-branch_id = "{encrypt_branch_id}" data-row-number="{record_number}">
...
<label class="col-sm-3 text-right badge badge-warning" for="edit_remark">ระบุเหตุผล :</label>
					...
					<input type="hidden" name="encrypt_branch_id" />
