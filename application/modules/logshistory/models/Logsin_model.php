 ...
 * @date 2020-06-05
 */
class Logsin_model extends MY_Model
{
	...
	public function create($post)
	{
		...
		$data = array(
				'log_user' => $post['log_user']
				,'log_ipaddress' => $post['log_ipaddress']
				,'log_date' => setDateToStandard($post['log_date'])
				,'log_text' => $post['log_text']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if($begin_date  != '' && $final_date != ''){
		 			$where	.= ($where != '' ? ' AND ' : '') . " log_date BETWEEN '$begin_date 00:00:00' AND '$final_date 23:59:59' " ;
		...
		if ($where != '') {
			$this->db->join('tb_members AS tb_members_1', "$this->my_table.log_user = tb_members_1.userid", 'left');
		...
		$this->db->select("$this->my_table.*, tb_members_1.username AS logUserUsername, tb_members_2.firstname AS Userfirstname, tb_members_3.lastname AS Userlastname  	 
				");
		$this->db->join('tb_members AS tb_members_1', "$this->my_table.log_user = tb_members_1.userid", 'left');
		$this->db->join('tb_members AS tb_members_2', "$this->my_table.log_user = tb_members_2.userid", 'left');
		$this->db->join('tb_members AS tb_members_3', "$this->my_table.log_user = tb_members_3.userid", 'left');
	...
	public function read_0ld($start_row = FALSE, $per_page = FALSE)
	{
		$search_field 	= $this->session->userdata($this->session_name . '_search_field');
		$value 	= $this->session->userdata($this->session_name . '_value');
		$value 	= trim($value);
		...
		$begin_date = date("Y-m-d H:i:s", '2020-06-01 00:00:00');
		$final_date = date("Y-m-d H:i:s", '2020-06-05 00:00:00');
		...
		$this->db->select("$this->my_table.*, tb_members_1.username AS logUserUsername
				");
		$this->db->join('tb_members AS tb_members_1', "$this->my_table.log_user = tb_members_1.userid", 'left');
