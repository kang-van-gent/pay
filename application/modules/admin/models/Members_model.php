...
class Members_model extends MY_Model
{
	...
	public function create($post)
	{
		...
		$data = array(
				...
				,'password' => pass_secure_hash($post['password'])
				,'prefix' => $post['prefix']
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_pername AS tb_pername_1', "$this->my_table.prefix = tb_pername_1.pre_id", 'left');
		...
		$this->db->join('tb_members AS tb_members_4', "$this->my_table.create_user_id = tb_members_4.userid", 'left');
		$this->db->join('tb_members AS tb_members_5', "$this->my_table.modify_user_id = tb_members_5.userid", 'left');
		...
		$this->db->select("$this->my_table.*, tb_pername_1.pre_name AS prefixPreName
				, tb_members_level_2.level_title AS levelLevelTitle
				, tb_department_3.dpm_name AS departmentIdDpmName
				, tb_members_4.firstname AS createUserIdFirstname, tb_members_4.lastname AS createUserIdLastname
				, tb_members_5.firstname AS modifyUserIdFirstname, tb_members_5.lastname AS modifyUserIdLastname
				");
		$this->db->join('tb_pername AS tb_pername_1', "$this->my_table.prefix = tb_pername_1.pre_id", 'left');
		$this->db->join('tb_members_level AS tb_members_level_2', "$this->my_table.level = tb_members_level_2.level_value", 'left');
		$this->db->join('tb_department AS tb_department_3', "$this->my_table.department_id = tb_department_3.dpm_id", 'left');
