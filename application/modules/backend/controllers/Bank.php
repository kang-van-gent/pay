 ...
 */
class Bank extends CRUD_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('backend/Bank_model', 'Bank');
		$this->data['page_url'] = site_url('backend/bank');
		...
		$js_url = 'assets/js_modules/backend/bank.js?ft='. filemtime('assets/js_modules/backend/bank.js');
	...
	public function search()
	{
		...
		$page_url = site_url('backend/bank');
		...
		$this->render_view('backend/bank/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Bank', 'url' => site_url('backend/bank')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('backend/bank/preview_view');
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('backend/bank/add_view'); 
	}
	...
	public function formValidate()
	{
		...
		$frm->set_rules('bank_name', 'bank_name', 'trim|required');

		$frm->set_message('required', '- กรุณากรอก ชื่อธนาคาร');
	...
	public function formValidateAdd()
	{
		...
		$frm->set_rules('bank_id', 'bank_id', 'trim|required');

		$frm->set_message('required', '- กรุณากรอก รหัสธนาคาร');
		

		$message_add  = '';
		if ($frm->run() == FALSE) {
			$message_add .= form_error('bank_id');
		}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->setPreviewFormat($results);
				$this->render_view('backend/bank/edit_view');
			}
		}
	}
	...
	{
		$error = '';
		$bank_id = ci_decrypt($data['encrypt_bank_id']);
		if($bank_id==''){
			$error .= '- รหัส bank_id';
		}
		return $error;
