...
class Branch extends CRUD_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('backend/Branch_model', 'Branch');
		$this->load->model('FileUpload_model', 'FileUpload');
		$this->data['page_url'] = site_url('backend/branch');		
		...
		$js_url = 'assets/js_modules/backend/branch.js?ft='. filemtime('assets/js_modules/backend/branch.js');
	...
	public function search()
	{
		...
		$page_url = site_url('backend/branch');
		...
		$this->render_view('backend/branch/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
						array('title' => 'Branch', 'url' => site_url('backend/branch')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('backend/branch/preview_view');
			}
		}
	}
	...
	public function preview_ajax($encrypt_id = "")
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->setPreviewFormat($results);
				$this->parser->parse('backend/branch/preview_view', $this->data);
			}
		}
	}
	...
	public function add()
	{
		...
		$this->render_view('backend/branch/add_view'); 
	}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			$this->data['message'] = "ไม่พบข้อมูลตามรหัสอ้างอิง <b>$id</b>";
				...
				$this->setPreviewFormat($results);
				$this->render_view('backend/branch/edit_view');
			}
		}
	}
