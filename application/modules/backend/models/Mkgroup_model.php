 ...
 * @date 2020-04-08
 */
class Mkgroup_model extends MY_Model
{
	...
	{
		$mkgroup_id = checkEncryptData($data['mkgroup_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.mkgroup_id = $mkgroup_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.mkgroup_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'mkgroup_name' => $post['mkgroup_name']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	...
	public function update($post)
	{
		...
		$mkgroup_id = checkEncryptData($post['encrypt_mkgroup_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.mkgroup_id = $mkgroup_id");
